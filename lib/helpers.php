...
// Returns the current user's storage folder (creates it if missing) and returns its real absolute path.
function user_root_dir(): string
{
    $u = current_user();
    ...
    $real = realpath($dir);
    if ($real === false)
        throw new RuntimeException("Cannot access user directory");
    return rtrim($real, DIRECTORY_SEPARATOR);

}

// Normalizes a relative path and blocks dangerous traversal like ".." or null bytes.
function normalize_rel_path(string $rel): string
{
    ...
    if (preg_match('~(^|/)\.\.(/|$)~', $rel))
        throw new RuntimeException("Blocked path traversal");
    return $rel;
}

// Validates a single file/folder name (not a full path) to prevent slashes or "..".
function safe_item_name(string $name): string
{
    $name = trim($name);
    if ($name === '')
...
// Resolves an existing relative path to a real absolute path inside the user's root directory.
function abs_path_existing(string $rel): string
{
    $base = user_root_dir();
...
// Builds an absolute path for a new item under an existing parent folder inside the user's root.
function abs_path_new(string $parentRel, string $name): string
{
    $base = user_root_dir();
    $parentRel = normalize_rel_path($parentRel);
    $parentAbs = $base . ($parentRel ? "/$parentRel" : '');
    $parentReal = realpath($parentAbs);
    if ($parentReal === false)
        throw new RuntimeException("Parent not found");
    $parentReal = rtrim($parentReal, DIRECTORY_SEPARATOR);
    if (strpos($parentReal, $base) !== 0)
        throw new RuntimeException("Forbidden");
